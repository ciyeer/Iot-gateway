...
### Fixed
- **Docker 构建脚本**：修复 `run_docker_build.sh` 中复杂的 Shell 转义错误，改用动态生成并挂载 `entrypoint_build.sh` 的方式，彻底解决了 `bash -c` 内嵌 heredoc 导致的语法问题。
- **Web 服务启动**：修复因 `start.sh` 中相对路径依赖 (`cd $(dirname $0)`) 在部分 Shell 环境下失效导致 `www_root` 路径解析错误的问题。
...
- **配置管理**：废弃 `start.sh` 中的动态 `sed` 替换逻辑，改为在构建阶段或运行时直接指定正确的配置文件路径。
...
### Changed
- **构建系统重构**：大幅优化 `build.sh` 脚本，集成了编译、清理和打包功能，移除了独立的 `package_rk3568.sh` 脚本，统一了操作入口。
  - 新增 `-p` / `--package` 参数：控制是否生成部署包（默认仅编译）。
  - 新增 `-x` / `--x86` 参数：明确指定 x86_64 架构构建。
  - 新增 `-a` / `--arm` 参数：明确指定 aarch64 架构构建（默认）。
  - 优化 `-c` / `--clean` 参数：支持精细化清理（可组合 `-a`, `-r` 清理特定架构或类型的构建目录）。
- **打包输出**：调整打包产物命名格式为 `iotgw_<arch>_<type>.tar.gz`（去除冗余的 `_deploy` 后缀），并将其输出路径调整至 `build/<arch>/<type>/deploy/`，彻底解决了根目录污染问题。
- **文档管理**：将 `PROJECT_STATUS.md` 归档至 `docs/` 目录，保持项目根目录清爽。
