...
static void HandleSignal(int) { RunningFlag().store(false); }
...
static void LoadDevicesFromConfig(const iotgw::core::common::config::ConfigManager& cfg,
                                  const std::string& topic_prefix, iotgw::core::device::manager::DeviceRegistry& out) {
    std::size_t i = 0;
    while (true) {
        ...
        d.telemetry_topic =
            topic_prefix.empty() ? (std::string("telemetry/") + id) : (topic_prefix + "telemetry/" + id);
    ...
    while (true) {
        ...
        d.telemetry_topic = topic_prefix.empty() ? (std::string("state/") + id) : (topic_prefix + "state/" + id);
        (void)out.Register(std::move(d));
        ++i;
    }
}
...
int GatewayCore::Run(const Args& args) {
    ...
    if (cfg.GetString("network.http_api.host", http_host) && !http_host.empty() &&
        ...
        web_opt.listen_addr = std::string("http://") + http_host + ":" + std::to_string(static_cast<int>(http_port));
    } else {
        std::string host2;
        std::int64_t port2 = 0;
        if (cfg.GetString("listen.host", host2) && !host2.empty() && cfg.GetInt64("listen.port", port2) && port2 > 0 &&
            port2 <= 65535) {
            web_opt.listen_addr = std::string("http://") + host2 + ":" + std::to_string(static_cast<int>(port2));
        }
    }
    ...
    iotgw::core::device::protocol_adapters::mqtt::MqttClient mqtt_client(web_server.GetMgr(), logger);
    ...
    if (mqtt_enabled) {
        ...
        if ((cfg.GetString("mqtt.broker_host", mqtt_host) && !mqtt_host.empty() &&
            ...
            (cfg.GetString("broker.host", mqtt_host) && !mqtt_host.empty() && cfg.GetInt64("broker.port", mqtt_port) &&
             mqtt_port > 0 && mqtt_port <= 65535)) {
            mo.url = std::string("mqtt://") + mqtt_host + ":" + std::to_string(static_cast<int>(mqtt_port));
        } else {
        ...
        if (((cfg.GetInt64("mqtt.keepalive_sec", keepalive)) || (cfg.GetInt64("client.keepalive_sec", keepalive))) &&
            keepalive > 0 && keepalive <= 65535) {
                ...
                rule_engine.OnSensorValue(device_id, sensor_value,
                                          [&](const iotgw::core::control::rule_engine::Rule& rule,
                                              ...
                                              if (action.type == "actuator_set") {
                                                  ...
                                                  const std::string lvl2 = ToLower(action.level);
                                                  const std::string msg2 = action.message.empty()
                                                                               ? (std::string("rule_fired: ") + rule.id)
                                                                               : action.message;
                                                  if (lvl2 == "warn" || lvl2 == "warning") {
                                                      logger->Warn(msg2);
                                                  } else if (lvl2 == "error") {
                                                      logger->Error(msg2);
                                                  } else if (lvl2 == "debug") {
