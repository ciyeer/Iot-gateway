...
bool MqttClient::Publish(const std::string& topic, const std::string& payload, std::uint8_t qos, bool retain) {
...
void MqttClient::SetMessageHandler(MessageHandler handler) { on_msg_ = std::move(handler); }
