...
public:
    ...
    using WsMessageHandler = std::function<void(struct mg_connection* c, const std::string& message)>;

    using HttpHandler = std::function<bool(struct mg_connection* c, struct mg_http_message* hm)>;
