...
CameraManager::~CameraManager() { StopStream(); }
...
bool CameraManager::StartStream(const std::string& device, const std::string& resolution, int fps, int port) {
    ...
    if (pid == 0) {
        // 子进程
        // 构建 mjpg_streamer 命令
        // mjpg_streamer -i "input_uvc.so -d /dev/video0 -r 640x480 -f 30" -o "output_http.so -p 8081 -w
        ...
        std::string output_args =
            "output_http.so -p " + std::to_string(port) + " -w /usr/local/share/mjpg-streamer/www";
...
bool CameraManager::StartRecording(const std::string& save_path, const std::string& device) {
    std::lock_guard<std::mutex> lock(record_mutex_);

    if (recording_ && CheckRecordingProcess()) {
        if (logger_) logger_->Info("Already recording");
        return true;
    }
    ...
    if (pid == 0) {
        // 使用 FFmpeg 调用 rkmpp 硬件编码进行 H.264 录制
        // -f v4l2: 指定输入格式
        // -i device: 指定摄像头设备
        // -c:v h264_rkmpp: 使用瑞芯微 MPP 硬件加速编码
        // -y: 覆盖已存在的文件
        std::string ffmpeg_cmd = "ffmpeg -f v4l2 -i " + device + " -c:v h264_rkmpp -y " + save_path;
        ...
        execl("/bin/sh", "sh", "-c", ffmpeg_cmd.c_str(), nullptr);
        exit(127);
    } else {
        record_pid_ = pid;
        recording_ = true;
        current_record_path_ = save_path;
        if (logger_) logger_->Info("FFmpeg recording started: " + save_path + " (PID: " + std::to_string(pid) + ")");
        return true;
    }
}
...
bool CameraManager::StopRecording() {
    ...
    // FFmpeg 接收到 SIGINT (Ctrl+C) 会优雅地保存视频索引并退出
    if (record_pid_ > 0) {
        kill(record_pid_, SIGINT);

        int status;
        // 等待最多 3 秒让 FFmpeg 完成收尾工作
        for (int i = 0; i < 30; ++i) {
        ...
        // 如果还没退出，强制杀死
        if (kill(record_pid_, 0) == 0) {
            kill(record_pid_, SIGKILL);
            waitpid(record_pid_, &status, 0);
        }
    }

    recording_ = false;
    record_pid_ = -1;
    if (logger_) logger_->Info("FFmpeg recording stopped");
    return true;
}

bool CameraManager::IsRecording() const { return recording_; }

bool CameraManager::IsRunning() const {
    // const_cast 用于在 const 函数中调用非 const 的 CheckProcess (虽然 CheckProcess
    // 逻辑上不应修改状态，但为了线程安全可能需要锁) 这里简化处理，直接返回 running_ 标志，配合定期检查
    return running_;
}
...
bool CameraManager::CheckRecordingProcess() {
    if (record_pid_ <= 0) return false;
    return (kill(record_pid_, 0) == 0);
}
...
void CameraManager::KillRecordingProcess() {
    if (record_pid_ > 0) {
        // FFmpeg 接收到 SIGINT 会保存视频头
        kill(record_pid_, SIGINT);

        int status;
        int ret = waitpid(record_pid_, &status, WNOHANG);

        if (ret == 0) {
            // 等待 2 秒让 FFmpeg 优雅退出
            for (int i = 0; i < 20; i++) {
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                if (waitpid(record_pid_, &status, WNOHANG) > 0) return;
            }
            kill(record_pid_, SIGKILL);
            waitpid(record_pid_, &status, 0);
        }
